...
In a bid to improve the results from the logistic regression, we tried an SVM model with a linear kernel and the found the best cost value was 1. We first had to change the outcome variable to a factor. The model used 138 support vectors, and though the best model output claims the overall test error rate was 11.2%, the manual calculation using the actual predicted values gave a test error rate of 13.8%. Additionally, the accuracy for the songs that made it into the top 20 was even lower at 24% so it didn't do as well as we had hoped.

Though none of the models did extremely well, we choose the tree boosting model as our final model because it had the highest classification accuracy for top 20 songs, even though it had a higher overall error rate than logistic and SVM (see table below).
...
Our final model step was to apply our chosen tree boosting model to full dataset, first converting the top 20 variable back to a logical type.
...
The results were not significantly different from those obtained from the test data, though they were slightly better, with an overall error rate of 13.0% and a top 20 accuracy rate of 56%.
